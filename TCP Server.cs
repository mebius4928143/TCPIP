		...
		internal bool Open(string ipAddress, int port, int listen, int buffersize)
		{
			...
			if (isOpen == false)
			{
				...
				else
				{
					...
					if (isMessage == true)
						_ = System.Windows.Forms.MessageBox.Show("IPアドレス文字列が不適切です", "TCP Server オープン", (MessageBoxButtons)MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
					return false;
				}
				...
				if (ipAddress != "127.0.0.1")
				{
					if (new List<IPAddress>(Dns.GetHostAddresses(Dns.GetHostName())).ConvertAll(x => x.ToString()).Any(l => l == ipAddress) == false)
					{
						...
						if (isMessage == true)
							_ = System.Windows.Forms.MessageBox.Show("指定されたIPアドレスは存在しません。", "TCP Server オープン", (MessageBoxButtons)MessageBoxButton.OK, (MessageBoxIcon)MessageBoxImage.Error);
						return false;
					}
				}
